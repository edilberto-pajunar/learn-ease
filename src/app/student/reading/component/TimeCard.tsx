...
const TimeCard: React.FC<TimeCardProps> = ({ material }) => {
  const { setDuration, setMiscues, clearMiscues } = useReadStore()
  const [startTime, setStartTime] = useState<number | null>(null)
  const [endTime, setEndTime] = useState<number | null>(null)
  const [isReading, setIsReading] = useState(false)
  const [miscues, setLocalMiscues] = useState<string[]>([])
  ...
  useEffect(() => {
    ...
    setLocalMiscues([])
    setShowText(false)
    clearMiscues() // Also clear global miscues
  ...
  const handleWordTap = (word: string, index: string) => {
    if (!isReading) return

    // Toggle miscue - if word is already marked as miscue, remove it
    if (miscues.includes(word)) {
      setLocalMiscues(miscues.filter((w) => w !== word))
    } else {
      // Add word to miscues if not already there
      setLocalMiscues([...miscues, word])
    }
  }

  const getWordStyle = (word: string, index: string) => {
                    ...
                    <div key={lineIndex} className="mb-2">
                      {words.map((word, wordIndex) => (
                        <span
                          ...
                          onClick={() =>
                            handleWordTap(word, `${lineIndex}-${wordIndex}`)
                          }
                          className={`mr-1 ${getWordStyle(word, `${lineIndex}-${wordIndex}`)}`}
                          title={
            ...
            {!startTime ? (
              ...
              <>
                ...
                <p className="text-muted-foreground max-w-md mx-auto">
                  Click "Start Reading" to begin. Read at your own pace and tap
                  any words you mispronounce or struggle with. This helps track
                  your reading accuracy and timing.
                </p>
              </>
            ) : (
              // Reading finished
              <>
                ...
                <p className="text-muted-foreground max-w-md mx-auto">
                  Great job! You've finished reading the passage. You identified{' '}
